...
.sv-cal td{border:1px solid var(--hairline);height:52px;vertical-align:top;padding:3px 4px;font-size:10.5px;cursor:pointer;border-radius:4px}
.sv-cal td:hover{background:rgba(146,193,220,.12)}
.sv-cal td.off{background:var(--surface-3);cursor:default}
.sv-cal td.trip{background:rgba(145,165,198,.16)}
.sv-cal .dts{display:flex;gap:2px;flex-wrap:wrap;margin-top:2px}
.sv-cal .dt{width:8px;height:8px;border-radius:50%;border:1px solid rgba(20,20,19,.25)}
.sv-legend{display:flex;gap:12px;flex-wrap:wrap;font-size:11px;margin-top:8px;align-items:center}
.sv-legend .dt{display:inline-block;width:9px;height:9px;border-radius:50%;margin-right:4px;vertical-align:-1px;border:1px solid rgba(20,20,19,.25)}
...
let svSeq = 1, svTripSel = null, svCalMonth = null, svMarkAs = '';
...
window.renderVisits = function(){
  ...
  const tripSpans = trips.filter(t=>t.start&&t.end).map(t=>({s:t.start, e:t.end}));
  ...
  for (let w=0; w<6 && day<=daysIn; w++){
    ...
    for (let dd=0; dd<7; dd++, day++){
      ...
      const inTrip = tripSpans.some(sp=>date>=sp.s && date<=sp.e);
      cells += `<td class="${inTrip?'trip':''}" onclick="svMarkDay('${date}')" title="Click to mark ${esc(svMarkAs||me)}: available → unavailable → clear">
        <div>${day}</div>
        <div class="dts">${marks.map(a=>`<span class="dt" title="${esc(a.person)} — ${a.state==='yes'?'available':'unavailable'}" style="background:${a.state==='yes'?svColor(a.person):'#fff'};border-color:${svColor(a.person)}"></span>`).join('')}</div>
      </td>`;
    }
    cells += '</tr>';
  }
      ...
      <div>
        <div class="sv-box">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;flex-wrap:wrap;gap:6px">
            ...
            <span style="font-size:11.5px;display:flex;gap:5px;align-items:center">Marking as
          ...
          <div class="sv-legend">
            ${svPeople().filter(p=>VISITS.avail.some(a=>a.person===p||a.person===p.replace(' (DP)','')) || p===me).slice(0,8).map(p=>`<span><span class="dt" style="background:${svColor(p)}"></span>${esc(p)}</span>`).join('')}
            <span class="muted">filled dot = available · hollow = unavailable · shaded day = trip scheduled</span>
          </div>
        </div>
        <div class="sv-box">
